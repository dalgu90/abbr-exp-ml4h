...
flags.DEFINE_integer("cv_idx", 0, "10-fold CV round (0~9)")

flags.DEFINE_string("dataset", "train", "which split to use (train/dev/test/traindev/all)")
...
class UMNCVProcessor(DataProcessor):
    def get_train_examples(self, data_dir, cv_idx):
        """See base class."""
        cv_indices = [(cv_idx + r) % 10 for r in range(2, 10)]
        return [e for idx in cv_indices for e in self._create_examples(
            self._read_tsv(os.path.join(data_dir, "cv", f"{idx}.tsv")), "train")]

    def get_dev_examples(self, data_dir, cv_idx):
        """See base class."""
        idx = (cv_idx + 1) % 10
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "cv", f"{idx}.tsv")), "dev")

    def get_traindev_examples(self, data_dir, cv_idx):
        """See base class."""
        cv_indices = [(cv_idx + r) % 10 for r in range(1, 10)]
        return [e for idx in cv_indices for e in self._create_examples(
            self._read_tsv(os.path.join(data_dir, "cv", f"{idx}.tsv")), "train")]

    def get_test_examples(self, data_dir, cv_idx):
        """See base class."""
        idx = cv_idx
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "cv", f"{idx}.tsv")), "test")
...
def convert_single_example_train(ex_index, example, max_seq_length, max_n_masks, tokenizer):
    tokens_left = tokenizer.tokenize(example.text_left)
    tokens_right = tokenizer.tokenize(example.text_right)
    tokens_pos = tokenizer.tokenize(example.label)
    tokens_negs = [tokenizer.tokenize(neg) for neg in example.negs]
    tokens_abbr = tokenizer.tokenize(example.group)

    tokens_left_abbr = tokens_left.copy()
    tokens_right_abbr = tokens_right.copy()
    _truncate_seq_pair(tokens_left_abbr, tokens_right_abbr, max_seq_length - len(tokens_abbr) - 2)
...
def convert_single_example_test(ex_index, example, max_seq_length, max_n_masks, tokenizer):
    tokens_left = tokenizer.tokenize(example.text_left)
    tokens_right = tokenizer.tokenize(example.text_right)
    tokens_label = tokenizer.tokenize(example.label)
    tokens_negs = [tokenizer.tokenize(neg) for neg in example.negs]
    tokens_abbr = tokenizer.tokenize(example.group)
    label_abbr_negs = [tokens_label] + [tokens_abbr] + [t_neg for t_neg in tokens_negs]

    features = []
    for tokens in label_abbr_negs:
        ...
        # input_ids_sub, input_mask_sub, segment_ids_sub, positions_sub, masks_mask_sub = _make_sentence(
            # tokens_left_2, tokens_right_2, tokens, max_seq_length, max_n_masks, tokenizer)
...
def file_based_convert_examples_to_features(examples, max_seq_length, max_n_masks, tokenizer, output_file, is_training):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if is_training:
            features = convert_single_example_train(ex_index, example, max_seq_length, max_n_masks, tokenizer)
        else:
            features = convert_single_example_test(ex_index, example, max_seq_length, max_n_masks, tokenizer)
...
def convert_single_result(example, arrays, tokenizer):
    tokens = [
    ...
    # _, vector_abbr = _take(arrays, 1, tokens[1])
    result['log_probs_label'] = _take(arrays, 0, tokens[0])
    # result['log_probs_label'], vector_label = _take(arrays, 0, tokens[0])
    # result['inner_label'] = np.inner(vector_abbr, vector_label)
    for i in range(len(example.negs)):
        result[f'log_probs_neg_{i}'] = _take(arrays, i + 2, tokens[i + 2])
        # result[f'log_probs_neg_{i}'], vector_neg = _take(arrays, i + 2, tokens[i + 2])
        # result[f'inner_neg_{i}'] = np.inner(vector_abbr, vector_neg)
...
def main(_):
    ...
    processor = UMNCVProcessor()
    dataset_fname = f'{FLAGS.dataset}.tf_record'
    if FLAGS.dataset == "train":
        examples = processor.get_train_examples(FLAGS.data_dir, FLAGS.cv_idx)
    elif FLAGS.dataset == "dev":
        examples = processor.get_dev_examples(FLAGS.data_dir, FLAGS.cv_idx)
    elif FLAGS.dataset == "traindev":
        examples = processor.get_traindev_examples(FLAGS.data_dir, FLAGS.cv_idx)
    elif FLAGS.dataset == "test":
        examples = processor.get_test_examples(FLAGS.data_dir, FLAGS.cv_idx)
    elif FLAGS.dataset == "all":
        processor = UMNProcessor()
        dataset_fname = f'dev.tf_record'
    ...
    if FLAGS.do_train:
        ...
        file_based_convert_examples_to_features(
            examples, FLAGS.max_seq_length, FLAGS.max_n_masks, tokenizer,
            os.path.join(FLAGS.output_dir, dataset_fname), True)
        input_fn = file_based_input_fn_builder(
            input_file=os.path.join(FLAGS.output_dir, dataset_fname),
        ...
        file_based_convert_examples_to_features(
            examples, FLAGS.max_seq_length, FLAGS.max_n_masks, tokenizer,
            os.path.join(FLAGS.output_dir, dataset_fname), False)
        input_fn = file_based_input_fn_builder(
            input_file=os.path.join(FLAGS.output_dir, dataset_fname),
            seq_length=FLAGS.max_seq_length,
            max_n_masks=FLAGS.max_n_masks,
    ...
    if FLAGS.do_train:
        ...
        for i, prediction in enumerate(result):
            if prediction['guid'][0] != last_guid:
                outputs.append(convert_single_result(examples_dict[last_guid], lst, tokenizer))
                lst = []
